...
var MouseFollower = Class.extend({
	...
	// the initial position of el, relative to the offset parentOrg. made to match the initial offset of sourceEl
	top0: null,
	left0: null,
	...
	constructor: function(sourceEl, options) {
		...
		this.parentEl = options.parentEl ? $(options.parentEl) : sourceEl.parent(); // default to sourceEl's parentOrg
	},
		...
		if (levelLimit && levelLimit < rowStruct.segLevels.length) { // is it actually over the limit?
			...
			for (i = 0; i < levelSegs.length; i++) {
				...
				if (totalSegsBelow) { // do we need to replace this segment with one or many "more" links?
					td = cellMatrix[levelLimit - 1][seg.leftCol]; // the segment's parentOrg cell
...
function computeSlotSegPressures(seg) {
	...
	if (seg.forwardPressure === undefined) { // not already computed
		...
		for (i=0; i<forwardSegs.length; i++) {
			...
			// either use the existing maximum, or use the childOrg's forward pressure
